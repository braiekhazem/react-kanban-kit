  ...
  return (
    <div className="trello-example-new-card">
      ...
      <div className="trello-example-new-card-buttons">
        <button onClick={() => addCardHandler(columnId, newCardTitle)}>Add</button>
        <button onClick={() => removeCardPlaceholderHandler(columnId)}>Cancel</button>
      </div>
    </div>
  );
};
...
const TrelloCard = ({ data }: { data: any }) => {
  return (
    <div className="trello-card">
      {data.content?.coverImage && (
        <div className="trello-card-cover">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={data.content.coverImage} alt="Card cover" draggable={false} />
        </div>
      )}
      <div className="trello-card-content">
        ...
        {data.content?.members && data.content.members.length > 0 && (
          <div className="trello-card-members">
            {data.content.members.slice(0, 4).map((member: string, index: number) => (
...
export default function TrelloBoard() {
  ...
  return (
    <div className="trello-example board-area">
      <Kanban
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        renderListFooter={(column) => (
          ...
          >
            <div className="trello-example-list-footer-button">+ Add card</div>
